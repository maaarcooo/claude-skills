...
name: discuss
description: >
  Ask clarifying questions and discuss direction before starting work. Use when
  the user invokes $discuss or says "discuss-first", wants to discuss
  requirements, clarify scope, or asks Codex to ask questions before proceeding
  with a task. Natural phrasings include "let's discuss before proceeding",
  "ask me questions for clarification or direction", and "let's discuss and
  make a plan first". Do not use for general requests to explain or discuss a
  topic when no pending task is being held back.
---
...
- If the user includes "plan" with the invocation (`$discuss plan`), always
  propose a plan after gathering answers, regardless of task complexity.
